...
export default function Home() {
  return <main className="marketing-shell">
    <header className="marketing-nav"><Link className="brand" href="/"><span className="brand-spark">✦</span><span>AIchemist<small>Your AI teammate in the room</small></span></Link><nav><a href="#how-it-works">How it works</a><a href="#workspace-preview">Workspace</a></nav><div><Link className="nav-login" href="/login">Log in</Link><Link className="nav-cta" href="/workspace?new=1">Start a session <span>↗</span></Link></div></header>

    <section className="landing-hero">
      <div className="hero-copy"><p className="landing-kicker"><i>✦</i> COLLABORATION, RE-ALCHEMIZED</p><h1>Your next breakthrough needs <em>another mind</em> in the room.</h1><p>AIchemist is a visual workspace where humans and AI create, challenge, sketch, and make sense of ideas together.</p><div className="hero-actions"><Link className="hero-primary" href="/workspace?new=1">Start a new session <span>→</span></Link><Link className="hero-secondary" href="/workspace">Explore the demo board <span>↗</span></Link></div><div className="landing-members"><div className="landing-avatars"><span>H</span><span>S</span><span>✦</span></div><p>Built for the moment when a good idea needs<br />a thoughtful challenge.</p></div></div>
    ...
    <section className="landing-cta" id="workspace-preview"><div><p className="landing-kicker">READY WHEN THE ROOM IS</p><h2>Start with a blank canvas.<br /><em>Bring the first thought.</em></h2></div><Link href="/workspace?new=1">Create a session <span>→</span></Link></section>
