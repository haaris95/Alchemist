...
export default function Workspace({ boardId }: { boardId?: string }) {
  ...
  return <main className="app-shell">
    ...
    <section className="workspace" id="board"><div className="board-column">
      ...
      <section className="webmcp-card"><div><span className="webmcp-mark">⌘</span><span><strong>WebMCP</strong><small>{mcpStatus === "ready" ? "13 native tools registered" : "Feature detection enabled"}</small></span></div><p>AI agents can inspect, create, move, edit, comment, map, sketch, connect, and remove board content through the same live session state.</p></section></> : <section className="webmcp-monitor-card" role="tabpanel"><div className="section-heading"><div><p className="eyebrow">WEBMCP OBSERVER</p><h2>Tool activity</h2></div><span>{board.webmcpActivity.length}</span></div><p className="webmcp-monitor-copy">Every successful WebMCP call appears here and is saved with the room.</p>{board.webmcpActivity.length ? <ol className="webmcp-tool-feed">{board.webmcpActivity.map((item) => <li key={item.id}><span className="webmcp-tool-icon">⌘</span><div><code>{item.tool}</code><p>{item.detail}</p></div><time>{item.timestamp}</time></li>)}</ol> : <p className="webmcp-empty">Waiting for a WebMCP-capable agent to use a tool in this room.</p>}</section>}</aside></section>
