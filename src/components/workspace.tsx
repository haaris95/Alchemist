...
type PitchIntent = "starter" | "independent" | "challenge";
...
export default function Workspace({ boardId }: { boardId?: string }) {
  ...
  function createSession(event: FormEvent) {
    ...
    boardStore.createSession(sessionName, currentMember.id); setSelectedNoteId(null); setSessionName(""); setShowNewSession(false); window.history.replaceState({}, "", "/workspace");
  }
  ...
  return <main className="app-shell">
    ...
    </div><aside className="sidebar"><section className={`ai-presence ${board.aiStatus === "thinking" ? "ai-presence--thinking" : ""}`}><div className="ai-presence-head"><span className="ai-orb">✦</span><div><strong>AIchemist</strong><p>{board.aiStatus === "thinking" ? "Thinking with the board" : autoPitch ? "Autonomous Groq teammate" : "Available on request"}</p></div><span className="presence-status">{board.aiStatus === "thinking" ? "WORKING" : autoPitch ? "AUTO" : "ACTIVE"}</span></div><p className="ai-presence-copy">{board.aiStatus === "thinking" ? "Looking for the missing idea or assumption worth challenging." : autoPitch ? "I’ll seed a blank room, introduce independent directions, and respectfully challenge weak assumptions about every two minutes." : "I’ll wait for you to invite me into the conversation."}</p><label className="autopilot-switch"><span><i>✦</i> Let AIchemist pitch in autonomously</span><input type="checkbox" checked={autoPitch} onChange={(event) => boardStore.setAiAutonomy(event.target.checked)} /><b></b></label><button className="pitch-button" disabled={board.aiStatus === "thinking"} onClick={() => void performPitchIn(new AbortController().signal, "challenge").catch(() => undefined)}>{board.aiStatus === "thinking" ? <><span className="thinking-ring"></span> Thinking…</> : <><span>✦</span> Ask AIchemist to challenge or pitch in</>}</button><small className={`pitch-note ${aiError || syncError ? "pitch-note--error" : ""}`}>{aiError ?? syncError ?? (isPersistent ? "This board is shared and saved live." : "Local preview — create a saved session to share it.")}</small></section>
