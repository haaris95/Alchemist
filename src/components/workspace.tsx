...
export default function Workspace({ boardId }: { boardId?: string }) {
  ...
  const [sidebarTab, setSidebarTab] = useState<"room" | "webmcp">("room");
  ...
  const performPitchIn = useCallback(async (signal: AbortSignal, intent: PitchIntent = "independent") => {
    ...
    try {
      ...
      const recordInAppTool = (tool: string, detail: string) => {
        boardStore.recordWebMCPTool(tool, detail, "in-app");
        setSidebarTab("webmcp");
      };
      recordInAppTool("pitch_in", "Groq reviewed the room and prepared an AIchemist contribution.");
      const result = boardStore.createNote({ text: contribution.text, color: contribution.color, authorId: "aichemist" });
      recordInAppTool("create_note", "Groq added an AIchemist sticky note.");
      ...
      if (connection) recordInAppTool("create_connection", "Groq connected two AIchemist board ideas.");
      if (strokes.length) recordInAppTool("draw_stroke", "Groq added " + strokes.length + " freehand " + (strokes.length === 1 ? "stroke." : "strokes."));
      if (diagram) recordInAppTool("create_diagram", "Groq created a deterministic visual map.");
  ...
  const onToolUsed = useCallback((entry: { tool: string; detail: string }) => {
    boardStore.recordWebMCPTool(entry.tool, entry.detail);
    setSidebarTab("webmcp");
  }, []);
  ...
  return <main className="app-shell">
    ...
    <section className="workspace" id="board"><div className="board-column">
      ...
      <section className="webmcp-card"><div><span className="webmcp-mark">⌘</span><span><strong>WebMCP</strong><small>{mcpStatus === "ready" ? "13 native tools registered" : "Feature detection enabled"}</small></span></div><p>AI agents can inspect, create, move, edit, comment, map, sketch, connect, and remove board content through the same live session state.</p></section></> : <section className="webmcp-monitor-card" role="tabpanel"><div className="section-heading"><div><p className="eyebrow">AI + WEBMCP OBSERVER</p><h2>Tool activity</h2></div><span>{board.webmcpActivity.length}</span></div><p className="webmcp-monitor-copy">Shows each board-changing action, labelled as in-app Groq or WebMCP.</p>{board.webmcpActivity.length ? <ol className="webmcp-tool-feed">{board.webmcpActivity.map((item) => <li key={item.id}><span className="webmcp-tool-icon">⌘</span><div><div className="webmcp-tool-name"><code>{item.tool}</code><small className={item.source === "in-app" ? "webmcp-tool-source webmcp-tool-source--groq" : "webmcp-tool-source"}>{item.source === "in-app" ? "GROQ · IN-APP" : "WEBMCP"}</small></div><p>{item.detail}</p></div><time>{item.timestamp}</time></li>)}</ol> : <p className="webmcp-empty">Waiting for AIchemist or a WebMCP-capable agent to use a board tool.</p>}</section>}</aside></section>
